...
namespace Combat
{
    ...
    public sealed class SwordSlashTrail : MonoBehaviour, ICombatAttackFeedbackHook
    {
        ...
        [SerializeField, Range(8, 100)] private int maxSamples = 40;
        [SerializeField, Min(0f)] private float fadeOutDuration = 0.2f;   // how fast trail dissolves on Recovery
        [SerializeField] private bool alwaysVisible;                      // emit continuously, not just during Slash
        ...
        public void OnCombatAttackPhase(CombatAttackFeedbackContext context)
        {
            if (alwaysVisible)
            {
                // Track slash state so we keep emitting, but never stop.
                slashing = context.Phase == CombatAttackPhase.Slash;
                return;
            }
        ...
        private void LateUpdate()
        {
            ...
            if (!alwaysVisible && fadingOut && now - fadeStartTime >= fadeOutDuration)
            {
                fadingOut = false;
                samples.Clear();
            }
            ...
            if (!alwaysVisible && fadingOut && fadeOutDuration > 0f)
